...
class Base:
    ...
    def perform(self, source, target, location):
        ...
        if len(self.sourceItemDelta) > 0 and not source.updateInventory(self.sourceItemDelta, target, location): return "{0} cannot be performed by {1} on {2}".format(self.name,source.name,(target.name if target else source.name))
        if len(self.targetItemDelta) > 0 and not target.updateInventory(self.targetItemDelta, target, location): return "{0} cannot be performed by {1} on {2}".format(self.name,source.name,(target.name if target else source.name))
